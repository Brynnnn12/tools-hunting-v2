...
from modules.sources import alienvault, leakix, securitytrails, shodan, urlscan
...
class SubdomainModule:
    ...
    def __init__(self, domain: str, api_keys: Optional[Dict[str, str]] = None) -> None:
        self.domain = domain
    ...
    def run(self) -> List[str]:
        ...
        try:
            results.update(alienvault.alienvault_passive_dns(self.domain, self.api_keys.get("ALIENVAULT_KEY", "")))
        except Exception:
            pass

        try:
            results.update(shodan.shodan_subdomains(self.domain, self.api_keys.get("SHODAN_KEY", "")))
        except Exception:
            pass

        try:
            results.update(securitytrails.securitytrails_subdomains(self.domain, self.api_keys.get("SECURITYTRAILS_KEY", "")))
        except Exception:
            pass

        try:
            results.update(leakix.leakix_subdomains(self.domain, self.api_keys.get("LEAKIX_KEY", "")))
        except Exception:
            pass

        try:
            results.update(urlscan.urlscan_subdomains(self.domain, self.api_keys.get("URLSCAN_KEY", "")))
        except Exception:
            pass
