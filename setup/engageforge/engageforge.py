...
def create_structure(base: Path, targets: List[str], no_notes: bool = False) -> Dict[str, List[str]]:
    ...
    for target in targets:
        ...
        if not env_file.exists():
            env_file.write_text(
                ENV_TEMPLATE.format(target=target, date=now, author=AUTHOR),
                encoding="utf-8",
            )
        ...
        if not no_notes:
            ...
            if not notes.exists():
                notes.write_text(
                    NOTES_TEMPLATE.format(target=target, date=now, author=AUTHOR),
                    encoding="utf-8",
                )
...
def main() -> int:
    ...
    # Interactive target input if -t not provided
    if args.targets:
        targets = [t for sublist in args.targets for t in sublist]
    else:
        raw = input("  Masukkan nama target: ").strip()
        if not raw:
    ...
    for label, name, tool_args in steps:
        answer = input(f"  Jalankan {label}? [Y/n]: ").strip().lower()
        if answer in ("", "y", "yes"):
            if label == "JSHunter":
                js_url = input("  URL file JS (enter untuk skip): ").strip()
                if js_url:
                    run_tool(name, ["-u", js_url, "--json"])
                    mark_env(target_dir, "JSHUNTER_DONE")
                else:
                    print("  [!] JSHunter dilewati\n")
                continue
